...
subroutine save1Dmathematica(time, fval, base_name, first_index)
    ...
    if (res_num.eq.1) then
        filename = base_name // '_1.csv' ! El nombre de salida + el subíndice que indica la resolución.
    else if (res_num.eq.2) then
        filename = base_name // '_2.csv'
    else if (res_num.eq.3) then
        filename = base_name // '_3.csv'
    else if (res_num.eq.4) then
        filename = base_name // '_4.csv'
    else if (res_num.eq.5) then
        filename = base_name // '_5.csv'
    end if
    ...
    ! Esto es lo que va a guardar la subrutina.
    do i=0,Nx,2**(res_num-1)
        write(1,'(1x, F, 3(",", F))') time, x(i), fval(i)
    end do 
    close(1)

end subroutine save1Dmathematica
